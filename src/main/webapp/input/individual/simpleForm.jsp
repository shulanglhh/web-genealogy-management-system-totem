...
String inputSimpleSortStr = (String)session.getAttribute("inputSimpleSortStr");
if(inputSimpleSortStr == null || inputSimpleSortStr.equals("")){
	inputSimpleSortStr = "ABYOPNQRTCXZKS";
}
%>
<style>
#simple-input-table .table-tr {	
	width:98%;
	height: 24px;
	font-family: 微软雅黑;
	font-size: 1em;
...
#simple-input-table input {
	height: 16px;
	font-size: 12px;
}

#simple-input-table .td1{
	width:85px;	
}
#simple-input-table .td2{
	width:70px;	
}
#simple-input-table .td3{
	width:400px;	
}

#simple-input-table .input-text {
	width:96%;
}
#simple-input-table .td4{
	width:40px;	
}
.chkBox{
...
<form id="simple-individual-form" method="post">
	<div class="easyui-panel" id="table-container" data-options="fit:true" style="overflow:hidden">
	<div id="buttonzwj" style="margin-top:10px">
		<a href="javascript:void(0)" class="easyui-linkbutton"
			onclick="submitForm(1, '#simple-individual-form')">保存(alt+1)</a> 
		<a href="#" class="easyui-linkbutton" onclick="submitForm(2, '#simple-individual-form')">添加子女(alt+2)</a>
		<a href="#" class="easyui-linkbutton" onclick="submitForm(3, '#simple-individual-form')">添加兄妹(alt+3)</a>
		<a href="#" class="easyui-linkbutton" onclick="submitForm(4, '#simple-individual-form')">添加配偶(alt+4)</a>
		<a href="#" class="easyui-linkbutton" onclick="submitForm(5, '#simple-individual-form')">添加侧室(alt+5)</a>
		<a href="#" class="easyui-linkbutton" onclick="openCreateFont()">造字(alt+6)</a>
		<a href="#" class="easyui-linkbutton" onclick="resetForm('#simple-individual-form')">重置</a>
		<a href="#" class="easyui-linkbutton" onclick="saveHkOrder()">保存录入顺序</a>
	</div>
	<table>
		<tr class="table-tr" data-itemidx="1">
			<td class="td1 chkBox" style="margin-right:10px;"><input name="hiddenchild" type="checkbox"/><label>不统计子女</label></td>
			<td class="td1 chkBox"><input name="inbiography" type="checkbox" checked="checked"/><label>是否在行传中出现</label></td>
			<td class="td1 chkBox"><input name="inrelation" type="checkbox" checked="checked"/><label>是否在吊线中出现</label></td>
			<!--<td class="td1 chkBox"><input name="hasmigrate" type="checkbox"/><label>是否外迁海外</label></td>-->
			</tr>
	</table>
		<table id="simple-input-table" style="width:610px;display:block;height:610px" onkeydown="Move()">
 			<% 
				int len = inputSimpleSortStr.length();
				String s1 = "<tr class=\"table-tr\" data-itemidx=\"",
					...
					s5 = "</td></tr>\n";
				for(int i=0; i<len; i++){
					Character c = inputSimpleSortStr.charAt(i);					
					String s = "";
				   ...
				   if(c=='A' || c=='a'){
					...
					/* else if(c=='l' || c=='L'){
						...
						cnt2 = "Alt+Shift+L";
						cnt3 = "出生时间 :";
						cnt4 = "<input class=\"mousetrap input-text\" name=\"birthday\" data-itemidx=\"Y\"></input>"+down+"birthday"+downup+"birthday"+up;
					} */
					if(!cnt4.equals("")){
	...
	<input name="otherinfo" type="hidden" value="" />
...
<script>
$("#simple-individual-form input, select").change(function(){			
	formChanged = true;	
});	
$(function() {
	...
	$('#simple-individual-form').form({//form中需要包括隐藏的关联人物信息{关联人物的id，父母id|关联人物的配偶id}
		...
		success : function(r) {
			...
			if (rObj.success) {//插入成功
				if(rObj.relInfo.curPid==0){
					$("#simple-individual-form")[0].reset();
...
/*/doesnt work well
$("#simple-input-table").dragsort({
	itemSelector: "#simple-input-table tr",
	dragSelector : "#simple-input-table tr",
	dragBetween : true,
	//dragEnd : updateOrder,
...
Mousetrap.bindGlobal('alt+shift+a', function() {focusInput('#simple-input-table input[name=surname]'); return false;});
Mousetrap.bindGlobal('alt+shift+b', function() {focusInput('#simple-input-table input[name=name]'); return false;});
Mousetrap.bindGlobal('alt+shift+e', function() {focusInput('#simple-input-table input[name=nickname]'); return false;});
//Mousetrap.bindGlobal('alt+shift+d', function() {focusInput('#simple-input-table input[name=huiname]'); return false;});	
Mousetrap.bindGlobal('alt+shift+c', function() {$("#simple-individual-form input[value='女']").attr("checked", true); return false;});
Mousetrap.bindGlobal('alt+shift+x', function() {$("#simple-individual-form input[value='false']").attr("checked", true);return false;});
Mousetrap.bindGlobal('alt+shift+p', function() {focusInput('#simple-input-table input[name=borndsc]'); return false;});
Mousetrap.bindGlobal('alt+shift+u', function() {focusInput('#simple-input-table input[name=deathdsc]'); return false;});
Mousetrap.bindGlobal('alt+shift+t', function() {focusInput('#simple-input-table input[name=remark]'); return false;});
//Mousetrap.bindGlobal('alt+shift+h', function() {focusInput('#simple-input-table input[name=huiname]'); return false;});
//Mousetrap.bindGlobal('alt+shift+g', function() {focusInput('#simple-input-table input[name=deathname]'); return false;});
//Mousetrap.bindGlobal('alt+shift+i', function() {focusInput('#simple-input-table input[name=ranknum]'); return false;});
//Mousetrap.bindGlobal('alt+shift+j', function() {focusInput('#simple-input-table input[name=title]'); return false;});
//Mousetrap.bindGlobal('alt+shift+m', function() {focusInput('#simple-input-table input[name=genword]'); return false;});
//Mousetrap.bindGlobal('alt+shift+n', function() {focusInput('#simple-input-table input[name=education]'); return false;});
Mousetrap.bindGlobal('alt+shift+o', function() {focusInput('#simple-input-table input[name=homeplace]'); return false;});
Mousetrap.bindGlobal('alt+shift+r', function() {focusInput('#simple-input-table input[name=officialtitle]'); return false;});
Mousetrap.bindGlobal('alt+shift+s', function() {focusInput('#simple-input-table input[name=lifedsc]'); return false;});
Mousetrap.bindGlobal('alt+shift+w', function() {focusInput('#simple-input-table input[name=graveyard]'); return false;});
Mousetrap.bindGlobal('alt+shift+q', function() {focusInput('#simple-input-table input[name=duty]'); return false;});
Mousetrap.bindGlobal('alt+shift+z', function() {focusInput('#simple-input-table input[name=endnote]'); return false;});
Mousetrap.bindGlobal('alt+shift+k', function() {focusInput('#simple-input-table input[name=othername]'); return false;});
//Mousetrap.bindGlobal('alt+shift+f', function() {focusInput('#simple-input-table input[name=otherinfo]'); return false;});
Mousetrap.bindGlobal('alt+shift+y', function() {focusInput('#simple-input-table input[name=rankdsc]'); return false;});
//Mousetrap.bindGlobal('alt+shift+l', function() {focusInput('#simple-input-table input[name=birthdaydetail]'); return false;});

Mousetrap.bindGlobal('alt+1', function() {$('#simple-individual-form input[name=name]').focus();submitForm(1, '#simple-individual-form'); return false;});
Mousetrap.bindGlobal('alt+2', function() {submitForm(2, '#simple-individual-form'); return false;});
Mousetrap.bindGlobal('alt+3', function() {submitForm(3, '#simple-individual-form'); return false;});
Mousetrap.bindGlobal('alt+4', function() {submitForm(4, '#simple-individual-form'); return false;});
Mousetrap.bindGlobal('alt+5', function() {submitForm(5, '#simple-individual-form'); return false;});
Mousetrap.bindGlobal('alt+6', function() {openCreateFont(); return false;});

/*Mousetrap.bind("alt+shift+a", function() {		
	focusInput('#simple-input-table input[name=surname]');
	return false;
});
Mousetrap.bind("alt+shift+e", function() {	
	focusInput('#simple-input-table input[name=nickname]');
	return false;
});*/

function hotkeyDown(s){
	var td = $("#simple-input-table tr td input[name='"+s+"']").parent();	
	var tr = td.parent();
...
function updateHkOrder(){
	var inputorder = "";
	$("#simple-input-table tr").each(function (){
...
function saveHkOrder(){
	...
	$.post('${pageContext.request.contextPath}/userAction!saveSimpleInputOrder.action', 
	...
	//$("#simple-input-table a[class='hk-order']").toggle();
}
function updateOrder() {
	var data = $("#simple-input-table tr").map(function() {
...
function Move()
{
	var rows=document.getElementById("simple-input-table").getElementsByTagName("input");
