...
$(function() {
   ...
   <body class="easyui-layout" data-options="fit:true">
    ...
    <div data-options="region:'center',title:'超级管理员管理'" style="padding:5px;background:#eee;">
    	<table id="cc" class="easyui-datagrid" data-options="url:'<%=path %>/userAction!showUsers.action',method:'post',border:false,singleSelect:true,toolbar:toolbar,pagination:true">
                <thead>
                    <tr>
       ...
       <div id="dlg" class="easyui-dialog" title="添加超级管理员" data-options="closed:true,buttons:[{
  					text:'提交',
  				iconCls:'icon-help',
  				handler:function(){
  				$('#newuser').submit();
          ...
          <form method="post" id='newuser'>
			<table>
    	       ...
    	       <div id="dlg2" class="easyui-dialog" title="修改超级管理员" data-options="closed:true,buttons:[{
  					text:'提交',
  				iconCls:'icon-help',
  				handler:function(){
  				$('#alteruser').submit();
    ...
    function tool(value,row,index){
        ...
        $(function() {
	        ...
	        cache:false,  
	        type: 'post', 
             url:'${pageContext.request.contextPath}/userAction!showUsers.action',//请求的action路径  
	        error: function () {//请求失败处理函数  
	            alert('获取用户列表失败');  
	        },  
        success:function(j){ //请求成功后处理函数。  
        	var rObj = eval('(' + j + ')');
	        		$("#cc").datagrid('loadData', rObj);
	        }  
	    });
        	*/
