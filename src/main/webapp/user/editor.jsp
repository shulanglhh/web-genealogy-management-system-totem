...
$(function() {
   ...
   <body class="easyui-layout" data-options="fit:true">
    ...
    <div data-options="region:'center',title:'谱志编辑管理'" style="padding:5px;background:#eee;">
    	<table id="cc" class="easyui-datagrid" data-options="url:'<%=path %>/userAction!showeditor.action',method:'post',border:false,singleSelect:true,toolbar:toolbar,pagination:true">
                <thead>
                    <tr>
       ...
       <div id="dlg" class="easyui-dialog" title="添加谱志编辑" data-options="closed:true,buttons:[{
  					text:'提交',
  				iconCls:'icon-help',
  				handler:function(){
  				$('#newuser').submit();
          ...
          <form method="post" id='newuser'>
			<table>
				...
				<input type="hidden" name="role" value="谱志编辑" />
				</td>
				</tr>
       ...
       <div id="dlg2" class="easyui-dialog" title="修改谱志编辑" data-options="closed:true,buttons:[{
  					text:'提交',
  				iconCls:'icon-help',
  				handler:function(){
  				$('#alteruser').submit();
    ...
    function tool(value,row,index){
        ...
        var toolbar = [{
            ...
            handler:function(){
            	...
            	$.post("${pageContext.request.contextPath}/userAction!deleditor.action",'uid='+obj.uid,function(data){
            		var j = eval('(' + data + ')');
            		if(j.msg=='ok')
            		{
            			$("#cc").datagrid("reload");
