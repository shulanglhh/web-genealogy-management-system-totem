...
$(function() {
	...
	$('.easyui-accordion').accordion('select', '族谱发布');
	...
	$('#volumelist_datagrid').datagrid({
		...
		detailFormatter:function(index,row){
			...
			return "<ul id='tree"+row.rank+"' class='ztree'></ul>";
		},
		...
		var treenode;
			...
			if(treenode!=undefined&&treenode!=null){
				...
				zTree = $.fn.zTree.init(ddv, setting,[]);
			}
...
function deleteVolume(){
	...
	$.messager.confirm('确定','您确定要删除该卷?',function(r){
	    if (r){
	    	if(delRow != null){
	...
	if(checkaddcontent()){
		...
		if(volrow!=null&&volrow!=undefined){
			...
			if(treeObj!=undefined){
				...
				volDG.datagrid('onExpandRow',volindex);
				treeObj = $.fn.zTree.getZTreeObj(treename);
				if(treeObj!=undefined){
					var treenode=treeObj.getSelectedNodes();
			...
			$('#volumelist_datagrid').datagrid('fixDetailRowHeight',volindex);
		}
		else{
			alert("请选择一个分卷！");
		}
...
function searchdoc(value){
	...
	$.ajax({
		...
		cache : false,
		type : 'post',
		data : 'docname='+value+'&gid='+gid,
		url : '<%=path%>/genealogyAction!searchthedoc.action',//请求的action路径  
		error : function() {//请求失败处理函数  
			alert('选择规则出错，请检查网络！');
		},
		success : function(j) { //请求成功后处理函数。  
			...
			if (rObj.success) {
				...
				$('#usersdoc_datagrid').datagrid('loadData', rObj.obj);
				$('#usersdoc_datagrid').datagrid('checkAll');
			} else {
				$.messager.show({
					title : '提示',
							...
							<div data-options="region:'center'" style="height:500px">
								<%--分卷列表--%>								
						         <table id="volumelist_datagrid" style="width:340px;height:480px"
							        ...
							        <thead>
							            <tr>
							                <th data-options = "field:'volname',fit:true" width="250">卷名</th>
					...
					<div class="easyui-panel" title="文档列表"
						...
						<div class="easyui-layout" data-options="fit:true">
							<div data-options="region:'north',border:false"
								...
								<div style="margin-top:10px">
								<label for="search-doc">输入文件名搜索：</label> 
								<input id="search-doc" class="easyui-searchbox"
							data-options="prompt:'输入文件名',width:'30px',searcher:searchdoc"></input>	
								</div>
								</div>
							<div data-options="region:'center'" style="height:500px">
								<%--文档列表--%>
