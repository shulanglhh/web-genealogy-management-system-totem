...
public class FileUploader
{
	...
	private void parseFormField(FileItem item, String encoding)
	{
		...
		if(!(value == null || value == "") && encoding != null)
		{
			try
			{
				value = new String(value.getBytes("UTF-8"), encoding);//or ISO-8859-1?
			}
	...
	private String getAbsoluteSavePath(HttpServletRequest request)
	{
		...
		if(!file.isAbsolute())
			file = new File(request.getRealPath(savePath));
		if(file.isDirectory())
